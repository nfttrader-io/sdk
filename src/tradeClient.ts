...
export default class TradeClient {
  ...
  constructor(config: {
    ...
    /**
     * Flag to avoid the private key when jsonRpcProvider is enabled.
     */
   ...
   * Create the swap
   *
   * @param createSwapObj - The createSwap configuration object
  ...
  public async createSwap(
    {
      ...
      /**
       * The ethereum amount provided by the creator of the swap.
       */
      ethMaker: string
      /**
       * The taker (counterparty) of the swap.
       */
      taker: string
      /**
       * The ethereum amount provided by the taker (counterparty) of the swap.
       */
      ethTaker: string
      /**
       * The number of the days representing the validity of the swap.
       */
      swapEnd?: number
      /**
       * The assets (ERC20/ERC721/ERC1155) provided by the creator of the swap.
       */
      assetsMaker?: any[]
      /**
       * The assets (ERC20/ERC721/ERC1155) provided by the taker (counterparty) of the swap.
       */
      assetsTaker?: any[]
      /**
    ...
    const addressMaker = this.isJsonRpcProvider
      ...
      : null
    if (!addressMaker)
      throw new Error("adressMaker not defined. check signer address.")
   ...
   * @param gasPrice - The gas price of the transaction.
   */
  public async estimateGasCreateSwap(
    {
      ethMaker,
      taker,
      ethTaker,
    ...
    try {
      const estimateGas = await contract?.estimateGas.createSwapIntent(
        swapIntent,
        assetsMaker,
        assetsTaker,
        referralAddress,
